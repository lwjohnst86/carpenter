...
make_factor_row <-
  function(data, header, rows, stat, digits) {
    ...
    factor_levels_df <-
      tibble::tibble(Variables = variable_names, Values = factor_levels) %>%
      dplyr::group_by(.data$Variables) %>%
      dplyr::mutate(ValOrder = 1:dplyr::n())

    variable_names_order <- tibble::tibble(
      Variables = rows,
    ...
    factor_pretable <- dplyr::full_join(
      factor_summary,
      tibble::tibble(VarOrder = 1:length(rows) - 0.5, Variables = rows),
